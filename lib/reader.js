...
PDFReader.prototype.render = function(pageNum, opt, callback) {
  ...
  pdf.getPage(pageNum).then(function(page) {
  	if(opt.scaleBounds){
  		var width = page.pageInfo.view[2],
  			height = page.pageInfo.view[3]
  		opt.scale = Math.min(opt.scaleBounds[0]/width, opt.scaleBounds[1]/height)
  	}
    ...
    page.render(renderContext).then(function() {
      ...
      var stream = canvas.pngStream().pipe(out);
...
PDFReader.prototype.renderAll = function(opt, callback, errorCb) {
  if (!this.pdf) {
    return this._pdfNotReady(callback);
  }
